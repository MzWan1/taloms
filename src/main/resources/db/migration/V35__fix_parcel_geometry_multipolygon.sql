-- V35__fix_parcel_geometry_multipolygon.sql
-- Fix: handle ST_MakeValid returning MultiPolygon when column type is Polygon
-- ST_MakeValid can return MultiPolygon for self-intersecting or invalid polygons,
-- which causes "Geometry type (MultiPolygon) does not match column type (Polygon)"
-- This rolls back the entire transaction, so boundary coordinates are never persisted.
